...
import { Trophy, ArrowRight, UserPlus, CheckCircle2, Edit3, Lock } from 'lucide-react';
...
export const ChallengeActions: React.FC<ChallengeActionsProps> = ({
  ...
  if (isCancelled) {
    ...
    if (hasIncompleteToday) {
      ...
      primaryIcon = <CheckCircle2 className="w-4 h-4 text-primary-foreground" aria-hidden="true" />;
    } else {
  ...
  return (
    <div className="flex items-center gap-2 pt-1">
      ...
      <button
        ...
        className="flex-1 min-h-[44px] px-4 py-2.5 rounded-xl bg-primary hover:bg-primary-hover active:bg-amber-600 text-primary-foreground font-bold text-xs tracking-wide shadow-xs transition-colors flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background"
      >
