...
interface ChallengeBasicsStepProps {
...
const CATEGORY_CARDS = [
  { id: 'Health', label: 'Health', icon: Dumbbell, desc: 'Fitness, workouts & vitality', token: '--cat-health' },
  { id: 'Study', label: 'Study', icon: BookOpen, desc: 'Learning, reading & research', token: '--cat-study' },
  { id: 'Work', label: 'Work', icon: Briefcase, desc: 'Deep focus, projects & craft', token: '--cat-work' },
  { id: 'Finance', label: 'Finance', icon: DollarSign, desc: 'Saving, investing & budgeting', token: '--cat-finance' },
  { id: 'Spiritual', label: 'Spiritual', icon: Sparkles, desc: 'Mindfulness, meditation & peace', token: '--cat-spiritual' },
  { id: 'Relationships', label: 'Relationships', icon: Heart, desc: 'Family, community & presence', token: '--cat-relationships' },
];

const PRESET_DURATIONS = [7, 14, 21, 30, 60];

export const ChallengeBasicsStep: React.FC<ChallengeBasicsStepProps> = ({
  draft,
  onChange,
  errors,
}) => {
  return (
    <div className="space-y-5 text-foreground">
      {/* Challenge Name */}
      <div>
        <label htmlFor="challenge-name-input" className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1.5">
        ...
        <div className="flex justify-between items-center mt-1">
          {errors.name ? (
            ...
            <span className="text-[11px] text-muted-foreground">A clear, motivating commitment name.</span>
          )}
          <span className="text-[11px] text-muted-foreground font-['JetBrains_Mono',monospace]">
            {draft.name.length}/60
          </span>
        </div>
      </div>

      {/* Category Picker with Tokenized Colors */}
      <div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${meta.badgeClass}`}>
      ...
      {/* Duration Chips & Date Range preview */}
      <div>
        <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 ml-1">
            ...
            <input
              ...
              className="w-16 px-2 py-1 text-xs text-center border border-border rounded-lg bg-surface-elevated text-foreground font-['JetBrains_Mono',monospace] focus:ring-primary focus:outline-none"
            />
            <span className="text-xs text-muted-foreground">days</span>
          </div>
        </div>

        {/* Derived Date Range Preview */}
        <div className="mt-3 p-3 rounded-xl bg-surface-elevated border border-border text-xs text-muted-foreground flex items-center justify-between">
          ...
          <span className="font-bold text-primary font-['JetBrains_Mono',monospace]">
