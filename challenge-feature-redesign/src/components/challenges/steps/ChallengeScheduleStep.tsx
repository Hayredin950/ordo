...
interface ChallengeScheduleStepProps {
...
export const ChallengeScheduleStep: React.FC<ChallengeScheduleStepProps> = ({
  draft,
  onChange,
  errors,
}) => {
  ...
  const daySchedule = draft.schedule.find((s) => s.dayOfWeek === activeDay) || {
    dayOfWeek: activeDay,
    blocks: [],
  };
  ...
  const handleAddBlock = (e: React.FormEvent) => {
    ...
    if (existingDayIndex >= 0) {
      updatedSchedule[existingDayIndex] = {
        ...updatedSchedule[existingDayIndex],
        blocks: [...updatedSchedule[existingDayIndex].blocks, newBlock],
      };
    } else {
    ...
    <div className="space-y-5 text-foreground">
      <div>
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-semibold text-primary font-['JetBrains_Mono',monospace]">
      ...
      {/* Time Block Adder for Active Day */}
      <form onSubmit={handleAddBlock} className="p-4 rounded-xl bg-surface-elevated border border-border space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="text-xs font-bold uppercase tracking-wider text-foreground">
            Add Block for {DAYS.find((d) => d.index === activeDay)?.full}
          </h4>
          <span className={`text-[11px] font-semibold px-2 py-0.5 rounded border ${catMeta.badgeClass}`}>
            {draft.category}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-6 gap-2">
          <div className="sm:col-span-3">
            <label className="block text-[11px] font-semibold text-muted-foreground mb-1">Activity Title</label>
            <input
              type="text"
              value={newTitle}
          ...
          <div className="sm:col-span-1.5">
            <label className="block text-[11px] font-semibold text-muted-foreground mb-1">Start</label>
            <input
              type="time"
              value={newStart}
              onChange={(e) => setNewStart(e.target.value)}
              className="w-full text-xs px-2 py-2 rounded-lg border border-border bg-surface text-foreground font-['JetBrains_Mono',monospace] focus:ring-primary focus:outline-none"
            />
          </div>

          <div className="sm:col-span-1.5">
            <label className="block text-[11px] font-semibold text-muted-foreground mb-1">End</label>
            <input
              type="time"
              value={newEnd}
      ...
      {/* Live Preview of resulting weekly routine */}
        ...
        <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
          {draft.schedule.length === 0 ? (
            ...
            draft.schedule.map((day) => (
              <div key={day.dayOfWeek} className="p-2.5 rounded-xl border border-border bg-surface-elevated space-y-1.5">
                <div className="flex items-center justify-between text-xs font-bold text-foreground border-b border-border/60 pb-1">
                  ...
                  <span className="text-[11px] text-muted-foreground font-normal">{day.blocks.length} block{day.blocks.length > 1 ? 's' : ''}</span>
                </div>
                <div className="space-y-1">
                  {day.blocks.map((b) => (
                    <div key={b.id} className="flex items-center justify-between text-xs p-1.5 rounded bg-surface border border-border/60">
                      <div className="min-w-0 pr-2">
                        <span className="font-semibold text-foreground">{b.title}</span>
                        <span className="text-muted-foreground text-[10px] ml-2 font-['JetBrains_Mono',monospace]">
