...
export const ChallengeReviewStep: React.FC<ChallengeReviewStepProps> = ({ draft }) => {
  const endDate = (() => {
    ...
    <div className="space-y-4 text-foreground">
      ...
      <div className="p-4 rounded-2xl bg-surface-elevated border border-border text-foreground shadow-xs space-y-3">
        <div className="flex items-start justify-between gap-3">
          <div>
            <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[11px] font-semibold border ${catMeta.badgeClass}`}>
        ...
        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border text-xs">
          <div>
            <span className="text-muted-foreground block text-[10px] uppercase">Schedule Period</span>
            <span className="font-semibold font-['JetBrains_Mono',monospace] text-foreground">
              {draft.startDate} → {endDate}
            </span>
          </div>
          <div>
            <span className="text-muted-foreground block text-[10px] uppercase">Weekly Load</span>
            <span className="font-semibold text-primary font-['JetBrains_Mono',monospace]">
      ...
      {/* Explicit Locking Warning Mandate */}
      <div className="p-3.5 rounded-xl bg-primary/10 border border-primary/25 text-primary flex items-start gap-3">
        ...
        <div className="text-xs space-y-1">
          ...
          <p className="text-muted-foreground leading-relaxed">
            <strong className="text-primary">"This routine becomes locked when the first member joins."</strong> Once another participant enrolls, schedule rules cannot be edited, ensuring mathematical parity in leaderboard scoring.
          </p>
        </div>
      </div>

      {/* Full Routine Breakdown Grouped by Day */}
          ...
          {draft.schedule.map((day) => (
            <div key={day.dayOfWeek} className="p-3 flex items-start justify-between text-xs">
              ...
              <div className="flex-1 space-y-1">
                {day.blocks.map((b) => (
                  <div key={b.id} className="flex items-center justify-between text-foreground bg-surface-elevated px-2.5 py-1.5 rounded-md border border-border/60">
