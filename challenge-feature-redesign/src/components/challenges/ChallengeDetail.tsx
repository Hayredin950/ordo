...
interface ChallengeDetailProps {
  ...
  onClose: () => void;
  onToggleBlock?: (challengeId: string, blockId: string) => void;
...
export const ChallengeDetail: React.FC<ChallengeDetailProps> = ({
  ...
  // Group routine by day of week (0 to 6)
        ...
        <div className="p-5 border-b border-border flex items-start justify-between gap-4 bg-surface-elevated/70">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2 flex-wrap">
              <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-md border ${catMeta.badgeClass}`}>
                {category}
              </span>
              <span className="text-border">•</span>
              <span className="text-xs text-muted-foreground flex items-center gap-1 font-['JetBrains_Mono',monospace]">
                <Calendar className="w-3.5 h-3.5 text-muted-foreground" aria-hidden="true" />
                {startAt} → {endAt} ({totalDays} Days)
              </span>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 p-3 rounded-xl bg-surface-elevated/60 border border-border">
            <div>
              ...
              <div className="text-2xl font-extrabold text-primary font-['JetBrains_Mono',monospace] mt-0.5">
            ...
            <div className="border-l border-border pl-3">
              ...
              <div className="text-2xl font-extrabold text-foreground font-['JetBrains_Mono',monospace] mt-0.5">
                {rank !== null ? `#${rank}` : '--'}
                <span className="text-xs font-normal text-muted-foreground"> / {memberCount || 1}</span>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {routineByDay.map(({ dayName, blocks }) => (
                  <div key={dayName} className="border border-border rounded-xl overflow-hidden bg-surface">
                    <div className="px-3.5 py-2 bg-surface-elevated border-b border-border flex items-center justify-between text-xs">
                      ...
                      <span className="text-[11px] text-muted-foreground font-medium">
                        {blocks.length === 0 ? 'Rest day' : `${blocks.length} scheduled block${blocks.length > 1 ? 's' : ''}`}
                      </span>
                    </div>
                      ...
                      {blocks.length === 0 ? (
                        ...
                        blocks.map((block) => {
                          ...
                          return (
                            <div 
                              key={block.id}
                              onClick={() => onToggleBlock?.(id, block.id)}
                              className="p-2 flex items-center justify-between hover:bg-muted/40 rounded-lg cursor-pointer transition-colors"
                            >
                              <div className="flex items-center gap-2.5 min-w-0 pr-2">
                                ...
                                <div className="min-w-0">
                                  <span className={`text-xs font-semibold ${block.completed ? 'line-through text-muted-foreground' : 'text-foreground'}`}>
                                    {block.title}
                                  </span>
                                  <div className="text-[10px] text-muted-foreground flex items-center gap-1.5 font-['JetBrains_Mono',monospace]">
                                    <Clock className="w-2.5 h-2.5 text-muted-foreground" aria-hidden="true" />
          ...
          {/* Tab 3: Detailed Score Metrics Breakdown */}
          {activeTab === 'breakdown' && (
            <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div>
                    <div className="flex justify-between text-xs font-semibold mb-1">
                      ...
                      <span className="font-['JetBrains_Mono',monospace] text-primary">{completion ?? 0}%</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-muted overflow-hidden">
                      <div className="h-full bg-primary rounded-full" style={{ width: `${completion ?? 0}%` }} />
                    </div>
                  </div>

                  <div>
                    <div className="flex justify-between text-xs font-semibold mb-1">
                      <span className="text-foreground">Consistency Score (Frequency & Streak)</span>
                      <span className="font-['JetBrains_Mono',monospace] text-primary">{consistency ?? 0}%</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-muted overflow-hidden">
                      <div className="h-full bg-primary rounded-full" style={{ width: `${consistency ?? 0}%` }} />
                    </div>
                  </div>

                  <div>
                    <div className="flex justify-between text-xs font-semibold mb-1">
                      <span className="text-foreground">Participation Index (Community & Logs)</span>
                      <span className="font-['JetBrains_Mono',monospace] text-primary">{participation ?? 0}%</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-muted overflow-hidden">
                      <div className="h-full bg-primary/70 rounded-full" style={{ width: `${participation ?? 0}%` }} />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="text-xs text-muted-foreground leading-relaxed bg-surface-elevated/40 p-3 rounded-lg border border-border">
                <strong>Platform Scoring Principle:</strong> Scores are calculated deterministically on the backend based on routine adherence, verified timestamped check-ins, and consistent streak velocity. The frontend never calculates or alters scoring algorithms.
              </div>
            </div>
          )}
        </div>
        ...
        <div className="p-4 border-t border-border bg-surface-elevated/70 flex items-center justify-between gap-3">
          ...
          {!joined && isUpcoming ? (
            <button
              ...
              className="min-h-[44px] px-5 py-2.5 rounded-xl bg-primary hover:bg-primary-hover active:bg-amber-600 text-primary-foreground font-bold text-xs tracking-wide shadow-xs transition-colors flex items-center gap-2"
            >
