...
export const ChallengeLeaderboard: React.FC<ChallengeLeaderboardProps> = ({
  ...
  const getRankBadge = (rank: number) => {
    ...
    return (
      <span className="w-6 h-6 rounded-full bg-muted/50 text-muted-foreground flex items-center justify-center font-semibold text-xs font-['JetBrains_Mono',monospace]">
        {rank}
      </span>
    );
  };
    ...
    <div className="space-y-4">
          ...
          {top3.map((member) => (
            ...
            >
              ...
              <div className="mt-1 w-full">
                ...
                <div className="text-sm font-extrabold text-foreground font-['JetBrains_Mono',monospace] mt-0.5">
      ...
      <div className="border border-border rounded-xl overflow-hidden bg-surface">
        <div className="bg-surface-elevated px-3 py-2 text-[11px] font-bold uppercase tracking-wider text-muted-foreground grid grid-cols-12 gap-2 border-b border-border">
          ...
          <span className="col-span-4 text-right">Score & Cons.</span>
        </div>
          ...
          {sorted.map((member) => (
            <div
              ...
              className={`px-3 py-2.5 grid grid-cols-12 gap-2 items-center text-xs transition-colors ${
                member.isCurrentUser ? 'bg-primary/10 font-semibold text-foreground' : 'hover:bg-muted/40 text-muted-foreground'
              }`}
            >
              ...
              <div className="col-span-4 text-right">
                <div className="font-bold text-foreground font-['JetBrains_Mono',monospace]">
