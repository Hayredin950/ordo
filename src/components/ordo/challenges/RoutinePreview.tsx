...
interface Props {
  routine: BlockWithDay[];
  compact?: boolean;
}

const DAYS = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];

export function ChallengeRoutinePreview({ routine, compact = true }: Props) {
              ...
              >
                <div className="min-w-0 pr-2">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {DAYS[b.dayOfWeek]} · {b.start} - {b.end}
                  </p>
                </div>
                <span
                  className={`text-[10px] font-medium px-2 py-0.5 rounded border shrink-0 ${catMeta.badgeClass}`}
                >
                  {b.category}
                </span>
              </div>
