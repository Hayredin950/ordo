...
import type { ChallengeCardModel, BlockWithDay } from "./types";
...
interface Props {
  ...
  onClose: () => void;
  onSaveRoutine: (challengeId: string, updatedRoutine: Record<number, Block[]>) => void;
}

export function ChallengeRoutineEditor({ challenge, onClose, onSaveRoutine }: Props) {
  const [blocks, setBlocks] = useState<BlockWithDay[]>(challenge.allWeeklyRoutine);
  ...
  const handleSave = () => {
    ...
    for (const block of blocks) {
      const { dayOfWeek: _day, ...blockWithoutDay } = block;
      if (!routineByDay[_day]) routineByDay[_day] = [];
      routineByDay[_day].push(blockWithoutDay);
    }
    onSaveRoutine(challenge.id, routineByDay);
    onClose();
  };
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {isLocked ? (
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                    This routine is locked because participants have already enrolled (
                    {challenge.memberCount} members). Modifying time blocks after start would
                    invalidate competitive ranking parity.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-primary/10 border border-primary/25 space-y-2">
              <p className="text-xs text-primary leading-relaxed">
                <strong>Routine is currently open for modification:</strong> You can add or adjust
                time blocks freely. The moment another member joins, the routine will permanently
                lock.
              </p>
            </div>
          )}
          ...
          {!isLocked && (
            <form
              ...
              className="p-4 rounded-xl bg-surface-elevated/60 border border-border space-y-3"
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full text-xs px-2.5 py-2 rounded-lg border border-border bg-surface text-foreground focus:outline-none focus:ring-2 focus:ring-primary font-mono"
                  />
                </div>
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <div className="text-[11px] text-muted-foreground flex items-center gap-2 mt-0.5 flex-wrap">
                          ...
                          <span>•</span>
                          <span
                            className={`inline-flex items-center gap-1 font-semibold ${catMeta.textClass}`}
                          >
